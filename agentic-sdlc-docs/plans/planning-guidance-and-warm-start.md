...
- Nothing was written to `agentic-sdlc-records/`. The last candidate plan and reviews exist only in the git-ignored
...
- The developer has no way to give the planner information it lacked. Re-running only re-rolls the dice.
  The only channel today is editing the Jira/Confluence sources (the fixture under `agentic-sdlc-local-inputs/PAY-DEMO-001/`),
...
| # | Decision | Detail |
|---|---|---|
| D1 | Durable, non-approvable evidence on non-convergence | Written to `agentic-sdlc-records/<ticket>/candidates/<run-id>/`, tracked in Git, state `NOT_CONVERGED`. `approve_plan.py` and Delivery never accept it. |
...
- Input: `guidance_file`, optional, declared as type `string` (a repo-relative or absolute file path). CAO's `path` input type only accepts existing *directories*
  ...
  Recommended location `agentic-sdlc-records/<ticket>/guidance.md`. An absent optional input is simply missing from `get_inputs()` (use `inputs.get(...)`).
- Format: free Markdown following `agentic-sdlc-docs/templates/developer-guidance.md` (new): sections for Decisions, Constraints, Clarifications, Answers to reviewer findings.
...
- Trust: the file is human-authored and read-only for agents (the write-scope hook already denies `agentic-sdlc-records/`). Document this trust decision in `governance.md` and `agentic-sdlc-docs/workflows/planning.md`.
...
- Only a reviewer `PASS` produces `agentic-sdlc-records/<ticket>/development-plan.md` and an `AWAITING_HUMAN_APPROVAL` manifest.
...
| `agentic-sdlc-docs/workflows/planning.md`, `agentic-sdlc-docs/architecture.md`, `agentic-sdlc-docs/README.md` | Usage, the non-convergence procedure, layout (`agentic-sdlc-records/<ticket>/candidates/`), link to this plan. |
| `agentic-sdlc-docs/templates/developer-guidance.md` | New template. |
...
| M1 candidate evidence | 3.1 for all four stop causes. | Unit tests per stop cause (force non-convergence through the fake-agent integration harness). **Live:** cold run with `max_review_rounds=1` on PAY-DEMO-001 (cheap, forces non-convergence) then inspect `agentic-sdlc-records/PAY-DEMO-001/candidates/<run-id>/`. |
...
- `execution-manifest.json` and the workflow output carry `guidance_sha256`; the exact file is published as `agentic-sdlc-records/<ticket>/plan-guidance.md`; candidate manifests carry it too.
...
- Governance invariants 16 (developer guidance) and 17 (review gate) were added; profile and contract text updated; docs and `agentic-sdlc-docs/templates/developer-guidance.md` written. The three planning profiles must be reinstalled before the next live run.
...
- Cold run `plan-PAY-DEMO-001-21` (`max_review_rounds=1`, `--wait --json`): stopped with `review_convergence_limit_reached`; candidate in `agentic-sdlc-records/PAY-DEMO-001/candidates/plan-PAY-DEMO-001-21/` with the index-prefixed review `reviews/01-review-r1-c1.json`. One blocking finding: `record_if_new` maps every `sqlite3.IntegrityError` (including NOT NULL on `payment_id`) to "duplicate".
- Synthetic guidance (`agentic-sdlc-records/PAY-DEMO-001/guidance.md`, labelled as an assistant-written test fixture, not a human decision) decided that only a PRIMARY KEY violation is a duplicate.
...
- Branch `feature/slim-agentic-sdlc`, commit `a806a98` (not merged to `main`). It moved docs, tools, examples and records out of `.agentic-sdlc/`.
  Records now go to `agentic-sdlc-records/<ticket>/`.
...
- Live verification of the `agentic-sdlc-records/` location is **still pending**: run 18 did not converge and run 19 was cancelled.
