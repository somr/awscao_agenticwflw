# Agentic SDLC Planning Workflow — Codex CLI Engineering Handoff
...
You are continuing development of the Agentic SDLC Planning Workflow in this repository.
...
1. Inspect the current repository state and existing tests.
2. Read `.agentic-sdlc/cao/workflows/dev_plan.py` fully.
3. Read the planning-agent profiles.
4. Read the existing contracts/schemas/templates referenced by `dev_plan.py`.
5. Reproduce or inspect the evidence from runs 5, 6, and 7 before redesigning the output path.
6. Keep CAO itself unmodified.
7. Do not silently broaden agent permissions.
8. Do not use terminal scraping as the final semantic-output contract.
...
The preferred architecture is a structured side channel, ideally a return-only capability, so agent semantic output is never reconstructed from tmux/ANSI/TUI state.
